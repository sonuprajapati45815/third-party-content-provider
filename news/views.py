...
def top_headline():
    ...
    redis_key = 'top_headline:category:{}:country:{}'.format(category, country)
    cached_news = redis_client.get(redis_key)
    if cached_news:
        return jsonify(json.loads(cached_news)), 200
    top_headline = get_news_by_top_headline(country, category)
    redis_client.set(redis_key, json.dumps(top_headline), ex=3600)
